...
SO="$ROOT/injector/bin/libchrome_hevc_trampoline_probe.so"
SCOUT_SO="$ROOT/injector/bin/libchrome_hevc_delegate_scout.so"
DELEGATE_SO="$ROOT/delegate-injection/bin/libh265_delegate_bundle_probe.so"
OFFSET_PROBE="$ROOT/injector/tools/probe_version_offsets.py"
...
for required in "$CHROME" "$SO" "$SCOUT_SO" "$DELEGATE_SO"; do
  if [[ ! -x "$required" ]]; then
